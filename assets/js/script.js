...
var getTrailer = function (movieTitle) {
  ...
  $.ajax(settings).done(function (trailerData) {
    ...
    var embedCode = trailerData.items[0].url.replace(
...
var showMovie = function (movieData) {
  ...
  $("#metacritic-rate").text(
    `${movieData.Ratings[2].Source} - ${movieData.Ratings[2].Value}`
  );
...
var showQuotes = function (quoteData) {
  ...
  quoteData.forEach((quoteItem) => {
    ...
    $(quoteText).html(
      `<h4 class='quote'>"${quoteItem.quote}"<br><br><span class='quote-character'>-${quoteItem.character}</span></h4><br>`
    );
